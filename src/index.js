...
// API路由：验证密码
router.post('/api/validate-password', async (request) => {
...
// API路由：获取剪贴板内容
router.get('/api/clipboard/:groupId', async (request) => {
...
// API路由：添加剪贴板内容
router.post('/api/clipboard/:groupId', async (request) => {
  const { groupId } = request.params;
  const data = await request.json();
  const { content } = data;
...
router.get('/debug', async () => {
  const initialized = await CLIPBOARD_DATA.get('static_initialized');
  const keys = await CLIPBOARD_DATA.list();
  
  const staticFiles = [];
  for (const key of keys.keys) {
    if (key.name.startsWith('static/')) {
      staticFiles.push(key.name);
    }
  }
  ...
  return new Response(JSON.stringify({
    ...
    staticFiles,
    allKeys: keys.keys.map(k => k.name)
  }, null, 2), {
    headers: { 'Content-Type': 'application/json' }
  });
});
...
addEventListener('fetch', (event) => {
  try {
    ...
    event.respondWith(new Response(`Fatal error: ${err.message}`, { status: 500 }));
  }
}); 